...
const ImagePuzzleModal = React.memo(
  ({ isOpen, onClose, puzzleImage = "/images/puzzle-image.jpg" }) => {
    ...
    const PuzzleSelectionMenu = React.memo(
      () => {
        ...
        const handleScroll = React.useCallback(() => {
          ...
          Array.from(scrollContainerRef.current.children).forEach(
            (card, index) => {
              const cardLeft = card.offsetLeft
              const cardWidth = card.offsetWidth
          ...
          // Only update index, don't auto-scroll
        ...
        // Initialize scroll position only once, preserve it on re-renders
        React.useEffect(() => {
          const container = scrollContainerRef.current
          if (container && !hasInitializedRef.current) {
            // First time initialization - don't reset scroll
            ...
            // Restore scroll position if it was saved and user isn't actively scrolling
          ...
          <div className="text-center">
            ...
            <div className="relative">
              ...
              <div className="mt-4 text-center">
                <p className="text-gray-800 dark:text-gray-200 text-sm font-medium bg-white/80 dark:bg-black/40 px-4 py-2 rounded-lg backdrop-blur-sm">
                  ← Veeg om door puzzels te bladeren →
                </p>
              </div>
            </div>
          </div>
...
// Only re-render if isOpen or onClose actually changes
